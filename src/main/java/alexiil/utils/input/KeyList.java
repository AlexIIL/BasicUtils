...
public class KeyList {
    ...
    /**
     * @param parent
     *            The parent to add a duplicate of all the keys from
     */
    public KeyList(KeyList parent) {
        for (Entry<Integer, IKeyInput> i : parent.keyInteger.entrySet()) {
            IKeyInput k = i.getValue();
            IKeyInput duplicate;
            ...
            keyInteger.put(i.getKey(), duplicate);
        }
    }
    ...
    /**
     * @param name
     *            The name of the key (to be saved in the config, and what to get it with)
     * @param k
     *            the key input type to add
     */
